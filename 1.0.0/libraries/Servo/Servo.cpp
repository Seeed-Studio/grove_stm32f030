...
void HANDLER_FOR_TIM(void) {
    	Servo_Handler(_timer, TIM_FOR_TIMER, CHANNEL_FOR_TIM);
}

void Servo_Handler(timer16_Sequence_t timer, TIM_TypeDef* tc, uint8_t channel)
{
	TIM_ClearITPendingBit(tc, TIM_IT_CC1);
	...
	tc->CCR1 = tc->CNT + SERVO(timer, Channel[timer]).ticks;
	...
	{ 
		...
		if( (tc->CNT) + 4 < usToTicks(REFRESH_INTERVAL) ) { // allow a few ticks to ensure the next OCR1A not missed
			tc->CCR1 = (unsigned int)usToTicks(REFRESH_INTERVAL);
		}
		else 
		{
            tc->CCR1 = tc->CNT + 4;  // at least REFRESH_INTERVAL has elapsed
		}
...
static void _initISR(TIM_TypeDef* tim, uint32_t channel, IRQn_Type irqn)
{
	...
	RCC_PeriphClock_EN_TIM;
	NVIC_InitStructure.NVIC_IRQChannel = IRQn_FOR_TIM;
...
{
    if (timer == _timer)
    	_initISR(TIM_FOR_TIMER, CHANNEL_FOR_TIM, IRQn_FOR_TIM);
} 

static void finISR(timer16_Sequence_t timer)
{
    TIM_Cmd(TIM_FOR_TIMER, ENABLE);     
}
